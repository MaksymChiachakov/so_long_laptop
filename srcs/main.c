/* main.c — so_long (single-file, fixed mlx_hook signatures) */
...
int main(int argc, char **argv)
{
    ...
    if (!data.mlx_ptr) { free_map(data.map, data.rows); return MLX_ERROR; }

    data.win_ptr = mlx_new_window(data.mlx_ptr, WINDOW_WIDTH, WINDOW_HEIGHT, "so_long");
    if (!data.win_ptr) { free_map(data.map, data.rows); return MLX_ERROR; }
    ...
    if (!data.background || !data.wall || !data.floor || !data.exit_close || !data.treasure || !data.exit_open || !data.player_img)
    {
